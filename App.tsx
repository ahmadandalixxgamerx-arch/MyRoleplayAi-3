...
import { generateResponse, summarizeChat, googleTranslateFree, conjureCharacter } from './services/apiService';
...
const renderFormattedContent = (content: string, settings: AppSettings, onImageClick?: (src: string) => void) => {
    ...
    // Use non-greedy matching to properly split thoughts (*) and dialogue (")
    ...
    return parts.map((part, index) => {
        ...
        if (part.startsWith('*') && part.endsWith('*')) {
            const thoughtColor = settings.thoughtColor || '#a1a1aa';
            return (
                <span key={index} style={{ color: thoughtColor }} className="italic opacity-90 italic-thought">
                    {part}
                </span>
            );
        } else if (part.startsWith('"') && part.endsWith('"')) {
            const dialogueColor = settings.dialogueColor || '#e4e4e7';
            return (
                <span key={index} style={{ color: dialogueColor }} className="font-medium dialogue-text">
                    {part}
                </span>
            );
        } else {
    ...
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-80 lg:w-96 bg-[#050505] border-r border-zinc-900/50 transform transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static flex flex-col shadow-[0_0_40px_rgba(0,0,0,0.5)] shrink-0`}>
        ...
        <div className="p-8 pb-4 flex items-center justify-between shrink-0">
          <h1 className="text-3xl font-serif font-bold text-red-600 tracking-widest drop-shadow-[0_0_20px_rgba(220,38,38,0.6)] animate-pulse-slow">
            EREBOS <span className="text-[10px] align-top bg-red-600 text-white px-1 rounded animate-bounce">REDESIGN V2</span>
          </h1>
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="space-y-1">
                ...
                <div 
                    ...
                    ${manageMode === 'chars' && selectedIds.has(char.id) ? 'bg-red-950/20 border-red-500/30' : ''}
                    ${!manageMode && activeCharId === char.id ? 'bg-zinc-900/40 border-red-900/30 shadow-[inset_0_0_20px_rgba(217,29,29,0.05)]' : 'hover:bg-zinc-900/30 hover:border-zinc-800'}`}
                >
                    {manageMode === 'chars' ? (
                         <div className={`text-red-500 transition-transform duration-300 ${selectedIds.has(char.id) ? 'scale-110' : 'opacity-50 scale-100'}`}>
                            {selectedIds.has(char.id) ? <CheckSquare size={16} /> : <Square size={16} />}
                         </div>
                    ) : (
                        <>
                            {activeCharId === char.id && (
                                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-red-500 shadow-[0_0_8px_rgba(217,29,29,0.8)] rounded-r"></div>
                            )}
                        </>
                    )}
                    
                    <div className="relative" onClick={(e) => { e.stopPropagation(); onViewImage(char.avatarUrl); }}>
                        <img src={char.avatarUrl} alt={char.name} className={`w-10 h-10 rounded-full object-cover transition-all duration-500 hover:scale-110 hover:ring-2 hover:ring-red-500/50 cursor-zoom-in ${activeCharId === char.id ? 'grayscale-0 ring-1 ring-red-500/50 shadow-[0_0_10px_rgba(217,29,29,0.2)]' : 'grayscale opacity-60 group-hover:opacity-100 group-hover:grayscale-0'}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h3 className={`font-serif font-medium truncate tracking-wide text-sm transition-colors duration-300 ${activeCharId === char.id ? 'text-red-100' : 'text-zinc-400 group-hover:text-zinc-200'}`}>{char.name}</h3>
            ...
            <div className="pt-2 space-y-2 animate-slide-up-fade">
               ...
               <div className="space-y-0.5">
                {charSessions.map((session: ChatSession) => (
                    ...
                    ${manageMode === 'sessions' && selectedIds.has(session.id) ? 'bg-red-950/20 text-red-400' : ''}
                    ${!manageMode && activeSessionId === session.id ? 'text-red-400 bg-red-950/10 border-l-2 border-red-500/50' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/20 border-l-2 border-transparent'}`}
                    >
                    {manageMode === 'sessions' && (
                         <div className={`mr-3 text-red-500 transition-transform duration-300 ${selectedIds.has(session.id) ? 'scale-110' : 'opacity-50 scale-100'}`}>
        ...
        <div className={`relative w-full transition-all duration-300 ${role === 'user' ? 'items-end' : 'items-start'}`}>
            <textarea
                ...
                className={`
                    ...
                    focus:ring-1 focus:ring-red-500/50 transition-all duration-200 shadow-xl
                    ${role === 'user' 
                        ? 'bg-zinc-900/60 text-zinc-200 border border-red-500/60 rounded-tr-sm' 
                        : 'bg-black/60 text-zinc-200 border border-red-500/30 rounded-tl-sm'
                    }
                `}
            />
            ...
            <div className="flex items-center justify-end gap-2 mt-3 animate-fade-in">
                <button 
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 text-[10px] font-bold uppercase tracking-wider text-zinc-400 hover:text-red-500 transition-all disabled:opacity-50"
                ...
                {role === 'model' && !isFirstMessage && (
                    <button 
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 text-[10px] font-bold uppercase tracking-wider text-red-500/80 hover:text-red-400 transition-all disabled:opacity-50"
                    >
                ...
                <button 
                    ...
                    className="flex items-center gap-1.5 px-4 py-1.5 rounded bg-red-700 hover:bg-red-600 text-white text-[10px] font-bold uppercase tracking-wider shadow-[0_0_15px_rgba(217,29,29,0.3)] hover:shadow-[0_0_25px_rgba(217,29,29,0.5)] transition-all border border-red-500/50"
                >
                    <Save size={10} /> Save
                </button>
            </div>
    ...
    return (
        <div className="mb-4 rounded bg-zinc-900/30 border border-zinc-800 overflow-hidden">
            <button 
                ...
                className="w-full flex items-center justify-between p-2 px-3 text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:text-red-500 hover:bg-zinc-800/50 transition-colors"
            >
                <div className="flex items-center gap-2">
...
function App() {
  ...
  const [activeCharId, setActiveCharId] = useState<string | null>(() => {
    return localStorage.getItem('erebos_activeCharId');
  });
  const [activeSessionId, setActiveSessionId] = useState<string | null>(() => {
    return localStorage.getItem('erebos_activeSessionId');
  });
  ...
  const handleStopGeneration = () => {
    ...
    const handleTranslateInput = async () => {
        if (!inputVal.trim() && !translation.original) {
            if (translation.original) {
                setInputVal(translation.original);
                setTranslation(prev => ({ ...prev, visible: false, original: '' }));
            }
            return;
        }

        if (translation.visible && translation.original) {
            setInputVal(translation.original);
            setTranslation({ visible: false, original: '', translated: '', loading: false, error: null });
            return;
        }
        ...
        try {
            const hasArabic = /[\u0600-\u06FF]/.test(currentVal);
            const target = hasArabic ? 'en' : 'ar';
            const result = await googleTranslateFree(currentVal, target);
  ...
  return (
    <div className="flex h-[100dvh] md:h-screen bg-[#030303] text-zinc-300 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col relative z-10 h-full">
                                        ...
                                        >
                                            ...
                                            {!manageMode && (
                                                <div className="flex items-center justify-end gap-2 mt-3 pt-3 border-t border-white/5 relative z-10 select-none">
                                                    ...
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); toggleMessageTranslation(msg.id, displayContent, msg.role); }} 
                                                        className={`p-1 rounded hover:bg-white/5 transition-all duration-300 flex items-center gap-1 ${translationState?.visible ? 'text-orange-500 bg-orange-500/10' : ''}`}
                                                        style={!translationState?.visible ? { color: settings.actionButtonColor, opacity: settings.actionButtonOpacity } : {}}
                                                        title={translationState?.visible ? "Restore Original" : "Translate"}
                                                    >
                                                        {translationState?.loading ? <Loader2 size={12} className="animate-spin" /> : translationState?.visible ? <RotateCcw size={12} /> : <Languages size={12} />}
                                                        {translationState?.visible && <span className="text-[8px] font-bold uppercase">Original</span>}
                                                    </button>
            ...
            <div className="p-6 md:p-8 pt-2 bg-gradient-to-t from-[#030303] via-[#030303] to-transparent relative">
                ...
                <div className="max-w-6xl mx-auto relative group mobile-safe-bottom pb-4 md:pb-0">
                ...
                <div className="relative flex items-end gap-2 bg-[#080808] border border-zinc-800/50 rounded-lg p-2 shadow-2xl transition-colors duration-300 group-focus-within:border-orange-900/30">
                    ...
                    <button
                        ...
                        disabled={translation.loading || (!inputVal.trim() && !translation.original)}
                        className={`
                            p-3 transition-all duration-300 rounded-lg flex items-center gap-2
                            ${(inputVal.trim() || translation.original) && !translation.loading
                                ? 'text-zinc-400 hover:text-orange-400 hover:bg-white/5'
                                : 'text-zinc-700 opacity-30 cursor-not-allowed'
                            }
                            ${translation.visible ? 'text-orange-500 bg-orange-500/10' : ''}
                        `}
                        title={translation.visible ? "Restore Original" : "Translate to English"}
                    >
                        {translation.loading ? <Loader2 size={20} className="animate-spin" /> : translation.visible ? <RotateCcw size={20} /> : <Languages size={20} strokeWidth={1.5} />}
                        {translation.visible && <span className="text-[10px] font-bold uppercase tracking-tighter">Undo</span>}
                    </button>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-fade-in">
             <div className="relative bg-[#050505] border border-zinc-800 w-full max-w-md p-8 shadow-[0_0_100px_rgba(234,88,12,0.15)] animate-modal-enter">
                ...
                <div className="text-center mb-8">
                    <h3 className="text-xs font-serif text-red-500 tracking-[0.3em] mb-2 uppercase drop-shadow-[0_0_8px_rgba(217,29,29,0.5)]">Manifestation // Character</h3>
                ...
                <div className="space-y-3">
                    <Button fullWidth variant="primary" onClick={handleStartNewChat} className="flex items-center justify-center gap-2 py-4 shadow-[0_0_20px_rgba(217,29,29,0.2)] hover:shadow-[0_0_30px_rgba(217,29,29,0.4)] transition-shadow duration-500">
      ...
      {sessionAction && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
              <div className="bg-[#0a0a0a] border border-red-900/30 p-6 rounded shadow-lg max-w-sm w-full m-4">
                  <h4 className="text-red-500 font-bold mb-4 uppercase tracking-wider">
                  ...
                  {sessionAction.type === 'rename' && (
                      <input 
                        className="w-full bg-black border border-zinc-800 p-3 text-zinc-200 focus:border-red-500/50 outline-none mb-6"
                        value={renameInput}
